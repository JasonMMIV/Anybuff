...
function buildPreviewSource(draft: AgentDraft): string {
  ...
  return [
    '// Generated by AnyBuff Agent Workshop',
    'import type { AgentDefinition } from "@codebuff/agent-runtime"',
    '',
...
export default function AgentWizardModal({ cwd, onClose, onCreated }: Props) {
  ...
  const save = async () => {
    ...
    try {
      ...
      const result = (await window.AnyBuff.createLocalAgent({ cwd, scope, ...draft })) as {
        ok: boolean
        id?: string
        filePath?: string
        error?: string
      ...
      <section className="settings-page-main agent-wizard-main">
        <header className="settings-page-header agent-wizard-header">
          <div>
            ...
            <p className="hint">{step === 1 ? 'Start from a proven role or make a specialist from scratch.' : step === 2 ? 'The prompts become the agent\'s operating contract.' : 'This is the exact definition AnyBuff will load into your project.'}</p>
          </div>
        </header>
