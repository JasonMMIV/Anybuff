...
export const PROVIDER_CONFIG_ENV_VAR = 'ANYBUFF_PROVIDER_CONFIG'
const PROVIDER_CONFIG_FILE_NAME = 'anybuff.json'
const GLOBAL_PROVIDER_CONFIG_FILE_NAME = 'provider-config.json'

/** Commands run after the agent edits files (e.g. typecheck/lint/test gates). */
...
export const providerConfigFileSchema = z
  ...
  .transform((config) => {
    ...
    const agentReasoningEfforts: Record<string, AnybuffReasoningEffort> = {
    ...
    const modeReasoningEfforts: Record<string, AnybuffReasoningEffort> = {
...
function readProviderConfigFile(
  ...
  try {
    ...
    const implicitDir = path.join(configDir, 'anybuff.d')
    if (fs.existsSync(implicitDir) && fs.statSync(implicitDir).isDirectory()) {
      if (
        !fragmentPaths.includes('anybuff.d') &&
        !fragmentPaths.includes('./anybuff.d')
      ) {
        fragmentPaths.push('anybuff.d')
      }
    }
...
function getAnybuffConfigDirs(): string[] {
  return [getConfigDir()]
}

function getDefaultProviderConfigPaths(): string[] {
  return [
    ...getAnybuffConfigDirs().flatMap((configDir) => [
...
export function getAncestorProviderConfigPaths(startDir: string): string[] {
  ...
  const trustAncestorConfig =
    (getSystemProcessEnv().ANYBUFF_TRUST_ANCESTOR_CONFIG ?? '') === '1'
...
function warnIfAncestorConfigHasApiKeyEnv(
  ...
  for (const provider of Object.values(config.providers ?? {})) {
    if (
      ...
      console.warn(
        `[anybuff] A provider config loaded from a non-project ancestor ` +
          ...
          `Set ANYBUFF_TRUST_ANCESTOR_CONFIG=1 to acknowledge and suppress this warning.`,
      )
      return
    }
  }
...
export function loadProviderConfigSync(
  ...
  if (!hasLoggedConfigPaths) {
    ...
    if (sourceFilePaths.length > 0) {
      console.info(
        `[anybuff] Loaded provider config from:\n  - ${sourceFilePaths.join('\n  - ')}`,
      )
    }
  }
...
export function resolveConfiguredProviderModel(params: {
  ...
  loadedConfig?: LoadedProviderConfig
  /** ADR-12: providerId → apiKey injected by the host. Wins over env vars. */
  apiKeyOverrides?: Record<string, string>
}): ResolvedProviderModel | undefined {
  const { model, env = getSystemProcessEnv(), apiKeyOverrides } = params
...
export type AnybuffProviderPreset = {
  id: string
  label: string
  description: string
...
export const ANYBUFF_PROVIDER_PRESETS = {
  'opencode-go': {
    id: 'opencode-go',
    label: 'OpenCode Go',
    description:
...
} satisfies Record<string, AnybuffProviderPreset>

export function createProviderPresetConfig(
  presetId: keyof typeof ANYBUFF_PROVIDER_PRESETS | string,
): ProviderConfigFile {
  const preset =
...
function tryWriteFragmentedConfig(
  ...
  const implicitDir = path.join(rootDir, 'anybuff.d')
  if (fs.existsSync(implicitDir) && fs.statSync(implicitDir).isDirectory()) {
    if (
      !fragmentPaths.includes('anybuff.d') &&
      !fragmentPaths.includes('./anybuff.d')
