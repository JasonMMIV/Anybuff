...
/**
 * Some OpenAI-compatible providers send billing telemetry frames
 * (`object: 'billing.summary'`) outside the normal chat choices stream.
 * They are not chat chunks and must never fail or alter the stream.
 */
function isOpenAICompatibleBillingSummaryChunk(value: unknown): boolean {
  return (
    value != null &&
    typeof value === 'object' &&
    'object' in value &&
    (value as Record<string, unknown>).object === 'billing.summary'
  )
}
      ...
      stream: response.pipeThrough(
        ...
        >({
          ...
          transform(chunk, controller) {
            ...
            if (!chunk.success) {
              // Tolerate benign non-chat frames: keep-alive nulls and billing
              // summary telemetry fail the chat chunk schema but must not
              // fail the stream.
              if (
                chunk.rawValue == null ||
                isOpenAICompatibleBillingSummaryChunk(chunk.rawValue)
              ) {
                return
              }
              finishReason = 'error'
            ...
            // Some OpenAI-compatible providers send successful billing
            // telemetry chunks outside the normal chat choices stream. Ignore
            // them without emitting response metadata or changing finish state.
            ...
            if (delta.tool_calls != null) {
              for (const toolCallDelta of delta.tool_calls) {
                ...
                if (toolCalls[index] == null) {
                  ...
                  // empty ("") or missing `function.name`. Treating that as a
                  // phantom call splits one logical call into two broken
                  // halves: the real call keeps truncated (unparsable)
                  // arguments and the phantom holds the orphaned suffix.
                  //
