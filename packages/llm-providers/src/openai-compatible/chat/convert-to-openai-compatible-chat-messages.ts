...
export function convertToOpenAICompatibleChatMessages(
  ...
  for (const { role, content, ...message } of prompt) {
    ...
    switch (role) {
      case 'system': {
        // Skip empty system prompts: some strict OpenAI-compatible endpoints
        // reject system messages whose content is empty or whitespace-only.
        if (
      ...
      case 'user': {
        ...
        // Strict OpenAI-compatible APIs reject the content-part array form
        // for plain text; flatten text-only user content to a string.
